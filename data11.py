...
if __name__ == '__main__':
    ...
    for product_id in pro_list:
        ...
        if len(x) > 3:
            ...
            if (y_train.values.sum() != 0) & (y_train.values.sum() != len(y_train)):
                ...
                x_all_pre = np.array([y_train.values[0]] * len(x))
            ...
            x_all_pre = np.array(np.floor(y_train.values.sum() / 2) * len(x))
            print '=3', y_pre
            x_pre_user['predict'] = y_pre
            x_user['predict'] = x_all_pre
        elif len(x) == 2:
            if y_train.values.sum() == 0:
                ...
                x_all_pre = np.array([0] * len(x))
            else:
                y_pre = np.array([1] * len(x_pre_user))
                x_all_pre = np.array([1] * len(x))
            print '=2', y_pre
            x_pre_user['predict'] = y_pre
            x_user['predict'] = x_all_pre
        else:
    ...
    # submission_all = submission_all.sort_values(['order_id'])
