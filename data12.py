...
def split_train_test(text_df, size=0.7):
    """
        分割训练集和测试集
    """
...
def model():
    ...
    lr = RandomForestClassifier(n_estimators=200, criterion='entropy')
...
if __name__ == '__main__':
    ...
    for product_id in pro_list:
        ...
        if len(xy) > 3:
            ...
            xy_train, xy_test = split_train_test(xy)
            y_train = xy_train['predict']
            x_train = xy_train.drop(['predict'], axis=1)
            y_test = xy_test['predict']
            x_test = xy_test.drop(['predict'], axis=1)
